...
final class ScreenCaptureDisplaySpaceRectMapperTests: XCTestCase {
    ...
    func testMapperPreservesDisplayOffsetAndScale() {
        let rect = ScreenCaptureDisplaySpaceRectMapper.displaySpaceRect(
            appKitRect: CGRect(x: -1_100, y: 500, width: 120, height: 40),
            appKitScreenFrame: CGRect(x: -1_200, y: 300, width: 600, height: 400),
            displayFrame: CGRect(x: 1_440, y: 0, width: 1_200, height: 800)
        )

        XCTAssertEqual(rect, CGRect(x: 1_640, y: 320, width: 240, height: 80))
    }
}
